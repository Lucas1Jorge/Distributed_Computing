...
func reduceFunc(input []mapreduce.KeyValue) (result []mapreduce.KeyValue) {
	...
	for _, keyValue := range input {
		...
		// map still don't have key
		if _, ok := mapAux[keyValue.Key]; !ok {
			mapAux[keyValue.Key] = 0
		}
		value, _ := strconv.Atoi(keyValue.Value)
		mapAux[keyValue.Key] += value
	}
